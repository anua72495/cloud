...
<body onload="setfocustocombo()">
	<form name="f1" id="f1" action="addSales.php" method="post">
...
<td align="center" valign='top'>
	<h2 align='center'><font face='TAHOMA' color='BLACK'>SALES DETAILS</font></h2>
...
$sql="Select Count(*) as Records From SalesMaster";
...
$qry="insert into SalesMaster(BillNo,BillDate,CustomerId,NetAmount) values(" . $RecNo . ",'" . date('Y-m-d') . "','" . $Customerid . "'," . "0" . ")";
...
$SNo=0;
$sql="Select count(*) Records From SalesTrans Where BillNo=" . $RecNo;
$res=mysqli_query($con,$sql) or die(mysql_error());
				$row = mysqli_fetch_assoc($res);
				$SNo=($row['Records']+1);
$qry="insert into SalesTrans values(" . $RecNo . "," . $SNo . ",'" . $ItemCode . "'," . $itemqty . "," . $itemrate . "," . $amount . "," . $tax . "," . $taxamount . "," . $netamount .")";
//echo $qry;
mysqli_query($con,$qry) or die(mysql_error());
	}	
echo "<h2><font color='White'>Sales Details Saved.</font></h2>";

$sql="Select Sum(NetTotal) Records From  SalesTrans Where BillNo=" . $RecNo ;
...
$sql="Update SalesMaster Set NetAmount=" . $BillAmount . " Where BillNo=" . $RecNo ;
	$res=mysqli_query($con,$sql) or die(mysql_error());
...
$sql="Select SNo,A.ItemCode,ItemName,Quantity,Rate,Amount,Tax,TaxValue,NetTotal From Items A,SalesTrans B where A.ItemCode=B.ItemCode and B.BillNo=" . $RecNo;
$res=mysqli_query($con,$sql) or die(mysql_error());
...
$sql="Select Sum(NetTotal) NetAmount From SalesTrans Where BillNo=" . $RecNo;
$res=mysqli_query($con,$sql) or die(mysql_error());
while($row = mysqli_fetch_assoc($res))
{
...
<tr><td>
			...
			<tr>
				...
				<tr>
					<td align="right">Qty
					<td><input name="txtQty"  style="text-align:right;" onblur="calc()" onkeypress="return isNumberKey(event)"/></td></tr>
				
				<tr>
					<td align="right">Rate
					<td><input name="txtRate"  style="text-align:right;background-color:#AAAAAA;color:white" onkeypress="javascript:return false;" value=<?php echo $cnt;?> onkeypress="return isNumberKey(event)"/></td></tr>
				
				<tr>
					<td align="right">Amount
					<td><input name="txtAmount"  style="text-align:right;background-color:#AAAAAA;color:white" onkeypress="javascript:return false;" value="0" onkeypress="return isNumberKey(event)"/></td></tr>
